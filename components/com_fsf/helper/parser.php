...
class FSFParser
{
	...
	function Load($template, $tpltype)
	{
		...
		if ($this->loadedtmpl == $template && $this->loadedtype == $tpltype)
			return;
		$db =& JFactory::getDBO();
		$query = "SELECT * FROM #__fsf_templates WHERE template = '" . $db->getEscaped($template) . "' AND tpltype = " . $db->getEscaped($tpltype);
		$db->SetQuery($query);
		$tmpl = $db->LoadObject();
	...
	function ParserPopulateTicket(&$parser, $row)
	{
		...
		if ($row)
		{
			...
			$parser->SetVar('subject',"<a href='".JRoute::_( '&limitstart=&ticketid=' . $row['id'] )."'>" . $row['title'] . "</a>");
			$parser->SetVar("status","<span style='color:" . $row['color'] . ";'>" . JText::_($row['status']) . "</span>");
						...
						if (array_key_exists($id,$this->customfields))
						{
							...
							if ($field['type'] == "plugin")
							{
								...
								{	
									...
									$value = $plugin->Display($value, $aparams['plugindata'], array('ticketid' => $row['id'], 'userid' => $row['user_id'], 'ticket' => $row));
								}
							}
			...
			$delete = "<a href='" . JRoute::_( 'index.php?view=admin&layout=support&tickets='.JRequest::getVar('tickets').'&delete=' . $row['id'] ) . "'>";
			$delete .= "<img src='" . JURI::root( true ) . "/components/com_fsf/assets/images/delete_ticket.png'>";
			$delete .= JText::_("DELETE") . "</a>";

			$archive = "<a href='" . JRoute::_( 'index.php?view=admin&layout=support&tickets='.JRequest::getVar('tickets').'&archive=' . $row['id'] ) . "'>";
			...
			{
				$trhl .= " style='cursor: pointer;' onclick='window.location=\"".JRoute::_( '&limitstart=&ticketid=' . $row['id'] )."\"' ";
			}
