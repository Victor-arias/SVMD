...
class FSF_ContentEdit
{
	...
	function Init()
	{
		if (empty($this->permission))
		{
			...
			$this->permission = FSF_Helper::getPermissions();		
	...
	function Display()
	{
		...
		if ($this->what == "cancel")
		{
			...
			$link = JRoute::_('index.php?view=admin&layout=content&type=' . $this->id,false);
			$return = JRequest::getVar('return','');
			if ($return && $return != 1)
				$link = $return;
	...
	function Save()
	{
		...
		{	
			...
			$this->item[$field->field] = JRequest::getVar($field->field,'');
			if ($field->type == "text")
				$this->item[$field->field] = JRequest::getVar($field->field, '', 'post', 'string', JREQUEST_ALLOWRAW);	
		...
		if ($ok)
		{
			...
			if ($this->item['id'])
			{
				...
				foreach ($this->edit as $edit)
				{
					$field = $this->GetField($edit);
					if ($field->type != "related")
						$sets[] = $field->field . " = '" . $db->getEscaped($this->item[$field->field]) . "'";
					if ($field->more)
						$sets[] = "`".$field->more . "` = '" . $db->getEscaped($this->item[$field->more]) . "'";
				}
				if ($this->permission['artperm'] > 2)
				{
					$sets[] = "published = " .JRequest::getVar('published',0);	
					$sets[] = "author = " . JRequest::getVar('author',0);
				}
				...
				$qry .= " WHERE id = '". $db->getEscaped($this->item['id']) . "'";
			} else {
				$fieldlist = array();
				$fieldlist[] = "author";
				if ($this->has_added)
				...
				foreach($this->edit as $edit)
				{
					...
					$setlist[] = "'" . $db->getEscaped($this->item[$field->field]) . "'";
					if ($field->more)
					{
						$fieldlist[] = "`".$field->more."`";	
						$setlist[] = "'" . $db->getEscaped($this->item[$field->more]) . "'";
					}
				...
				if ($this->permission['artperm'] > 2)
				{
					$setlist[] = JRequest::getVar('published',0);	
				} else {
					$setlist[] = "0";	
				}
			...
			foreach($this->edit as $edit)
			{
				...
				if ($field->type == "products")
				{
					...
					$qry = "DELETE FROM {$field->prod_table} WHERE {$field->prod_artid} = '{$db->getEscaped($this->item['id'])}'";
					$db->setQuery($qry);
					//echo $qry."<br>";
					$db->query($qry);
						...
						foreach ($this->products as &$product)
						{
							...
							if ($val == "on")
							{
								$qry = "INSERT INTO {$field->prod_table} ({$field->prod_prodid}, {$field->prod_artid}) VALUES
									($pid, '{$db->getEscaped($this->item['id'])}')";
								$db->setQuery($qry);
								//echo $qry."<br>";
								$db->query($qry);
							}
				...
				} elseif ($field->type == "related")
				{
					...
					$qry1 = "DELETE FROM {$field->rel_table} WHERE {$field->rel_id} = '". $db->getEscaped($this->item['id']) . "'";
					$db->setQuery($qry1);
					//echo $qry1."<br>";
					$db->query();
					...
					{
						$id = $db->getEscaped($id);
						$qry1 = "REPLACE INTO {$field->rel_table} ({$field->rel_id}, {$field->rel_relid}) VALUES ('". $db->getEscaped($this->item['id']) . "', '$id')";
						$db->setQuery($qry1);
						//echo $qry1."<br>";
						$db->query();
					}
			...
			if ($this->what == "apply")
			{
				$link = JRoute::_("index.php?option=com_fsf&view=admin&layout=content&type={$this->id}&what=edit&id={$this->articleid}",false);
			} elseif ($this->what == "savenew")
			{
				$link = JRoute::_("index.php?option=com_fsf&view=admin&layout=content&type={$this->id}&what=new",false);
			} else {
				$link = JRoute::_('index.php?view=admin&layout=content&type=' . $this->id,false);
				$return = JRequest::getVar('return','');
				if ($return && $return != 1)
	...
	function GetListFilter()
	{
		...
		{
			$this->filter_values[$filter->field] = $mainframe->getUserStateFromRequest($this->id.$filter->field,$filter->field,"");
	...
	function getList()
	{
		...
		if ($this->filter_values['published'] > 0)
			$where[] = "a.published = {$db->getEscaped($this->filter_values['published']-1)}";
		
		if ($this->filter_values['userid'] > 0)
			$where[] = "a.author = {$db->getEscaped($this->filter_values['userid'])}";
		...
		*/
		if ($this->filter_values['search'] != "")
		{
			...
			foreach($this->fields as $field)
			{
				if ($field->search)
				{
					$search[] = "{$field->field} LIKE '%{$db->getEscaped($this->filter_values['search'])}%'";	
				}	
			}	
		...
		foreach ($this->filters as $filter)
		{
			...
			if ($value > 0)
			{
				$where[] = "a.{$filter->field} = {$db->getEscaped($value)}";
			}
		}
		...
		{
			$qry .= " ORDER BY " . $db->getEscaped($this->filter_values['order']) . " " . $db->getEscaped($this->filter_values['order_dir']);
		} else {
	...
	function getSingle()
	{
		...
		foreach($this->edit as $edit)
		{
			...
			if ($field->type == "products")
			{
				$qry = "SELECT {$field->prod_prodid} FROM {$field->prod_table} WHERE {$field->prod_artid} = '{$db->getEscaped($id)}'";
				$db->setQuery($qry);
			...
			{
				$qry = "SELECT {$field->rel_relid} FROM {$field->rel_table} WHERE {$field->rel_id} = '{$db->getEscaped($id)}'";
				$db->setQuery($qry);
				$field->rel_ids = array();
	...
	function LookupInput($field, $item)
	{
		...
		return JHTML::_('select.genericlist',  $sections, $field->field, 'class="inputbox" size="1" ', $field->lookup_id, $field->lookup_title, intval($item[$field->field]));
	}
	...
	function GetLookupValues($field)
	{
		...
		{
			$ids[$item[$field->field]] = $item[$field->field];	
		}
		...
		$this->lookup_values[$field->field] = $db->loadAssocList($field->lookup_id);
	}
	...
	function HandleAuthor()
	{
		...
		if ($search != "")
		{
			$where[] = "(u.username LIKE '%{$db->getEscaped($search)}%' OR u.name LIKE '%{$db->getEscaped($search)}%' OR u.email LIKE '%{$db->getEscaped($search)}%')";
		}
	...
	function HandlePick()
	{
		...
		if ($this->filter_values['search'] != "")
		{
			...
			{
				$search[] = "{$searchfield} LIKE '%{$db->getEscaped($this->filter_values['search'])}%'";	
			}	
	...
	function HandlePublish()
	{
		...
		if ($this->permission['artperm'] > 2)
		{
			...
			$qry = "UPDATE {$this->table} SET published = $pub WHERE id = '{$db->getEscaped($id)}'";
	...
	function EditPanel($item,$changeclass = true)
	{
		...
		if ($this->permission['artperm'] > 1 || ($this->permission['artperm'] == 1 && $item['author'] == $this->userid)) {
			...
			echo "<a href='".FSFRoute::_("index.php?view=admin&layout=content&type={$this->id}&what=edit&id={$item['id']}&option=com_fsf&return=1")."' class='fsj_tip' title='".JText::_('EDIT_ARTICLE')."'><img src='" . JURI::root( true ). "/components/com_fsf/assets/images/edit.png' alt='Edit' /></a>";	
...
class FSF_Content_Field
{
	...
	function __construct($field, $desc, $type = "string")
	{
		$this->field = $field;
		$this->desc = $desc;
		$this->type = $type;
...
{
	function __construct($field,$sourcd_id,$source_display,$source_table,$source_order,$header)
	{
		$this->field = $field;
