...
class FsfsModelFaqs extends JModel
{
     var $_data;
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT f.id as id, question, answer, f.ordering as ordering, f.published as published, c.title as title FROM #__fsf_faq_faq as f LEFT JOIN #__fsf_faq_cat as c ';
        $query .= ' ON f.faq_cat_id = c.id ';

		$where = array();

        if ($this->lists['search']) {
			$where[] = '(LOWER( question ) LIKE '.$db->Quote( '%'.$db->getEscaped( $this->lists['search'], true ).'%', false ) . ')';
		}
