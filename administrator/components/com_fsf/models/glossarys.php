...
class FsfsModelGlossarys extends JModel
{
     var $_data;
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT  id, word, description, published FROM #__fsf_glossary ';

		$where = array();

        if ($this->lists['search']) {
			$where[] = '(LOWER( word ) LIKE '.$db->Quote( '%'.$db->getEscaped( $this->lists['search'], true ).'%', false ) . ')';
		}
